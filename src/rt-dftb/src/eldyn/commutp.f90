...
!-v2:
        call tstconv(dP,2*Nsq,N,thrs,ierr,err,tconv, iprint)
         errmax=max(errmax, abs(err))
        if (tConv) exit lp_bch 
        iter= iter +1 
      end do lp_bch
      write(*,*)'BCH converged: Niter, errmax =',  iter,errmax
...
!c-------TSTSCONV
subroutine  tstconv(C,M,N,thrs,ierr,err,tconv,iprint)
      implicit  none
      ...
      Ix = modulo(ierr-1,N)
      if (iprint> 1) write(*,1000)ix,iy,im,err,thrs
